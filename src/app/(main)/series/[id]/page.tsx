...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
    ...
    const title = `${series.title || series.name} ${year ? `(${year}) ` : ''}— все сезоны, актеры, отзывы, рейтинг`;
            ...
            <div className="relative z-20">
                <div className="absolute top-[100px] left-0 w-full z-30">
                    <div className="container">
                        <BackButton fallbackHref="/series" />
                    </div>
                </div>
                <section className='pt-[150px] max-[1100px]:pb-20 w-full min-[1100px]:min-h-screen md:min-h-[800px] flex flex-col justify-center'>
                    <div className="container flex max-[1100px]:flex-col gap-x-20 items-start">
                        <div className="max-[1100px]:mt-6 w-[30%] h-full max-[1100px]:w-full max-[1100px]:max-w-[360px] max-[1100px]:mx-auto relative pb-32">
                            <Image src={tmdbApi.getImageUrl(series.poster_path)} alt="Poster" width={500} height={750} className='rounded-xl w-full object-cover max-[1100px]:aspect-[9/16] min-[1100px]:h-full' />
                            <DetailActions
                                id={series.id}
                                type="tv"
