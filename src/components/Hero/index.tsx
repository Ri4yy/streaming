                ...
                <StaggerItem>
                    <div className="flex md:flex-col flex-row items-center md:items-start gap-x-2 mt-6 md:mt-0">
                        ...
                        <div className="flex items-center gap-x-2 md:mt-2">
                            <div className="typeMovie font-['SF_pro'] rounded-md bg-[#2BB157] w-fit px-2.5 py-1 text-white text-sm uppercase">{movie.media_type === 'tv' ? 'Сериал' : 'Кино'}</div>
                            <div className="typeMovie font-['SF_pro'] rounded-md bg-[#4A90E2] w-fit px-2.5 py-1 text-white text-sm uppercase">4K</div>
                        </div>
                    </div>
                </StaggerItem>
                <StaggerItem>
                    <h1 className='md:text-[50px] text-4xl font-bold w-fit my-4 max-w-4xl'>{movie.title || movie.name}</h1>
                </StaggerItem>
                <StaggerItem>
                    <div className="flex items-center">
                        ...
                        <span className='flex items-center relative before:w-1 before:h-1 before:bg-white/30 before:rounded-full before:absolute before:-left-1/2 ml-8 text-white/60'>{movie.release_date ? movie.release_date.split('-')[0] : (movie.first_air_date ? movie.first_air_date.split('-')[0] : 'N/A')}</span>
                    </div>
                </StaggerItem>
                <StaggerItem>
                    <p className='text-lg text-[#BFBFBF] my-4 lg:my-8 lg:w-1/2 md:w-4/5 leading-[1.2] line-clamp-3'>
                ...
                <StaggerItem>
                    <div className="flex items-center gap-4 mt-8">
                        <button className='flex gap-2 justify-center items-center group py-3 px-8 rounded-lg hover:scale-110 bg-[#ff1414] relative overflow-hidden transition-all duration-700'>
                            ...
                            <span className="relative z-10 group-hover:text-black transition-colors duration-500">Смотреть</span>
                        ...
                        <Link href={`/${movie.media_type === 'tv' ? 'series' : 'movies'}/${movie.id}`} className='hover:text-[#ff1414] transition-all duration-300'>Подробнее</Link>
                    </div>
                </StaggerItem>
            </StaggerContainer>
        </section>
