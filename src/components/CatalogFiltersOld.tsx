...
function CatalogFiltersOldContent({ genres, hideRating = false }: { genres: { id: number, name: string }[], hideRating?: boolean }) {
                                ...
                                <Listbox.Options className="absolute mt-2 max-h-60 w-full overflow-auto custom-scrollbar rounded-md bg-white py-2 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm text-black border border-gray-100">
                                    {sortOptions.map((option, index) => (
                                        <Listbox.Option
                                            key={index}
                                            className={({ active }) =>
                                                `relative cursor-default select-none py-2.5 pl-5 pr-4 ${active ? 'bg-gray-100 text-black' : 'text-gray-900'
                                                }`
                                            }
                                            value={option}
                                        >
...
export default function CatalogFiltersOld(props: { genres: { id: number, name: string }[], hideRating?: boolean }) {
    return (
        <React.Suspense fallback={<div className="h-20 w-full animate-pulse bg-white/5 rounded-lg"></div>}>
            <CatalogFiltersOldContent {...props} />
        </React.Suspense>
    );
}
